      ...
      {prescriptions.length === 0 ? (
        ...
        <div className="space-y-4">
          {prescriptions.map((presc, idx) => {
            // 🔥 SMART LOGIC: Check if AT LEAST ONE medicine in this prescription is still active
            const isPrescActive = presc.medicines && presc.medicines.some(med => {
              return getDurationStatus(presc.created_at, med.duration).status === 'active';
            });
            // If none are active (or no medicines exist), the overall prescription is inactive
            const prescStatus = isPrescActive ? 'active' : 'inactive';
              ...
              <motion.div key={presc.prescription_id || idx} variants={FADE_UP_SPRING} className="bg-white rounded-[24px] border border-slate-200/60 shadow-[0_2px_10px_-4px_rgba(0,0,0,0.02)] overflow-hidden">
                ...
                <div className="flex items-center justify-between p-6 cursor-pointer hover:bg-slate-50/50 transition-colors" onClick={() => setExpanded(expanded === idx ? null : idx)}>
                  <div className="flex items-center gap-4">
                    <div className={`w-12 h-12 border rounded-2xl flex items-center justify-center ${isPrescActive ? 'bg-sky-50 border-sky-100 text-sky-600' : 'bg-slate-50 border-slate-200 text-slate-400'}`}>
                      <Pill size={20} />
                    </div>
                    <div>
                  ...
                  <div className="flex items-center gap-4">
                    {/* 🔥 Dynamic Status Badge inserted here */}
                    <StatusBadge status={prescStatus} />
